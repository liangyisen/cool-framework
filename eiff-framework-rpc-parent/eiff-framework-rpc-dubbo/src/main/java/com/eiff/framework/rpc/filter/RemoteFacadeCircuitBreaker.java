...
import com.eiff.framework.rpc.exception.CircuitBreakerException;

/**
 * 通过拦截每个方法的请求，并且读取每个方法对服务降级条件的配置 从而自动进行服务降级，以及服务恢复
 */
@Deprecated
...
public class RemoteFacadeCircuitBreaker implements Filter {
	...
	public RemoteFacadeCircuitBreaker() {
		...
		logger.info("[{}] has already been initialized circuit breaker,check break marker interval [{}]", localHost,
				intervalConf);
	...
	private Result wrapBreakerInvoke(Invoker<?> invoker, Invocation invocation) throws RpcException {
		...
		if (checkNeedCircuitBreak(invoker, invocation)) {
			logger.info("[{}] activate the circuit break for url [{}],invoke method [{}]", localHost, invoker.getUrl(),
	...
	private boolean checkNeedCircuitBreak(Invoker<?> invoker, Invocation invocation) {
		...
		if (breakCounter != null && breakCounter.isEnable()) {
			...
			logger.info("[{}] check invoke [{}.{}] circuit break,current exception count [{}]  limit [{}]", localHost,
					interfaceName, method, currentExceptionCount, limit);
			if (limit <= currentExceptionCount) {
				if (currentBreakCount > 0 && needRetry(invoker, invocation, currentBreakCount)) {
					logger.info("[{}] retry invoke [{}.{}] current break count [{}]", localHost, interfaceName, method,
							currentBreakCount);
	...
	private boolean needRetry(Invoker<?> invoker, Invocation invocation, long currentBreakCount) {
		...
		logger.info("[{}] check invoke [{}.{}] need retry,current break count [{}],retry frequency [{}]", localHost,
				interfaceName, method, currentBreakCount, frequency);
	...
	private <T extends Object> Result doCircuitBreak(Invoker<?> invoker, Invocation invocation) throws RpcException {
		...
		CircuitBreakerException baseBusinessException = new CircuitBreakerException(interfaceName,
				invocation.getMethodName());
	...
	private void caughtException(Invoker<?> invoker, Invocation invocation, Exception e) {
		...
		methodConfig.append(".").append(method);
		String methodKey = methodConfig.toString();
		int timeout = invoker.getUrl().getMethodParameter(invocation.getMethodName(), Constants.TIMEOUT_KEY,
		...
		logger.info("[{}] caught exception for rpc invoke [{}.{}],current exception count [{}]", localHost,
				interfaceName, method, counter.getCurrentExceptionCount());
	}
}
