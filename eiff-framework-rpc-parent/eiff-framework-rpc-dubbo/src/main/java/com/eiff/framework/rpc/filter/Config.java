...
public class Config {
	...
	/**
	 * 获取某个方法或者某个接口的判定为出现异常的次数 提供在配置中心和dubbo.properties两种途径配置 如果两个地方均有配置，配置中心的为准
	 * 
	...
	public static int getBreakLimit(StringBuffer interfaceConfig, StringBuffer methodConfig) {
		...
		String breakLimitConf = ConfigUtils.getProperty(methodConfig.toString(),
				ConfigUtils.getProperty(interfaceConfig.toString(),
						ConfigUtils.getProperty("dubbo.reference.default.break.limit", DEFAULT_BREAK_LIMIT)));
		return Integer.parseInt(breakLimitConf);
	}

	/**
	 * 获取某个方法或者接口是否开启服务降级的功能，默认是开启降级功能 提供在配置中心和dubbo.properties两种途径配置
	 * 如果两个地方均有配置，配置中心的为准
	 * 
	 * @param interfaceConfig
	 * @param methodConfig
	...
	public static boolean getSwitch(StringBuffer interfaceConfig, StringBuffer methodConfig) {
		...
		String switchConfig = ConfigUtils.getProperty(methodConfig.toString(), ConfigUtils.getProperty(
				interfaceConfig.toString(), ConfigUtils.getProperty("dubbo.reference.circuit.break", "false")));
	...
	/**
	 * 获取某个方法或者接口重试频次，及每出现多少次异常，就重试一次远程接口 提供在配置中心和dubbo.properties两种途径配置
	 * 如果两个地方均有配置，配置中心的为准
	 * 
	 * @param interfaceConfig
	 * @param methodConfig
	...
	public static int getRetryFrequency(StringBuffer interfaceConfig, StringBuffer methodConfig) {
		...
		String retryFrequencyConf = ConfigUtils.getProperty(methodConfig.toString(),
				ConfigUtils.getProperty(interfaceConfig.toString(),
						ConfigUtils.getProperty("dubbo.reference.default.retry.frequency", DEFAULT_RETRY_FREQUENCY)));
