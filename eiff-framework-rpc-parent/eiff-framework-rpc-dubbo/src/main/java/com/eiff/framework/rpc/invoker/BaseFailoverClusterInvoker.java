...
public class BaseFailoverClusterInvoker<T> extends AbstractClusterInvoker<T> {

	private static final Logger logger = LoggerFactory.getLogger(FailoverClusterInvoker.class);

	public BaseFailoverClusterInvoker(Directory<T> directory) {
		super(directory);
	}
	...
	public Result doInvoke(Invocation invocation, final List<Invoker<T>> invokers, LoadBalance loadbalance)
		...
		for (int i = 0; i < len; i++) {
			...
			try {
				...
				if (i == len - 1) {
					return result;
				} else if (result != null && result instanceof RpcResult) {
					...
					if (resultObject instanceof BaseResponse) {
						...
						if (baseResponse.getRespCode().equals(CommonRspCode.SYS_TIMEOUT.getCode())) {
							continue;
						}
					}
				}
