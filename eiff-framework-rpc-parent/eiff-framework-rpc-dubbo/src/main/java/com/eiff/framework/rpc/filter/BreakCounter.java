...
class BreakCounter implements Runnable {
	...
	public void run() {
		...
		statisticsLogger.info(
				"[{}] checking [{}] break status,current exception times [{}] ,current break times [{}],retry times[{}]",
				localAddress, invoker, getCurrentExceptionCount(), getCurrentBreakCount(), getCurrentRetryTimes());
		while (markerIterator.hasNext()) {
			ExceptionMarker marker = markerIterator.next();
			logger.info("[{}] checking [{}] break marker whether expired  break at [{}],remain  [{}],case[{}]",
					localAddress, invoker, marker.getBreakTime(), marker.getRemain(), marker.getCaseByMessage());
			if (marker.isExpire()) {
				markerIterator.remove();
				decrementException();
			}
		}
		// statisticsLogger.info("[{}] current exception count
		// [{}]",localAddress,getCurrentExceptionCount());
	}
}