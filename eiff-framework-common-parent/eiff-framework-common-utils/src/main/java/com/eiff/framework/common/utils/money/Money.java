package com.eiff.framework.common.utils.money;
...
public class Money implements Serializable, Comparable<Object> {
    ...
    public boolean equals(Object other) {
        ...
        if (m.calculate_big_amount != null) {
            return currency.equals(m.currency)
                   && (getAmount().compareTo(m.calculate_big_amount) == 0);
        }
        return currency.equals(m.currency) && (calculate_amount == m.calculate_amount);
    }
    ...
    public boolean isIntegerValue() {
    	  return calculate_big_amount.signum() == 0 || calculate_big_amount.scale() <= 0 || calculate_big_amount.stripTrailingZeros().scale() <= 0;
    	}
}
