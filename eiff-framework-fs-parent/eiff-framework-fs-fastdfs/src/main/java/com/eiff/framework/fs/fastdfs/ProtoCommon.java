...
import com.eiff.framework.fs.fastdfs.vo.FileId;
...
public class ProtoCommon {
	 ...
	 * @return token string
	 */
	public static String getToken(String remote_filename, int ts, String secret_key, NonGlobalConfig config)
			throws UnsupportedEncodingException, NoSuchAlgorithmException, MyException {
		byte[] bsFilename = remote_filename.getBytes(config.getCharset());
		byte[] bsKey = secret_key.getBytes(config.getCharset());
		byte[] bsTimestamp = (new Integer(ts)).toString().getBytes(config.getCharset());
	...
	public static FileId splitFileId(String file_id) {
		int pos = file_id.indexOf(StorageClient.SPLIT_GROUP_NAME_AND_FILENAME_SEPERATOR);
		if ((pos <= 0) || (pos == file_id.length() - 1)) {
			return null;
		}
		return new FileId(file_id.substring(0, pos), file_id.substring(pos + 1));
	}
}
