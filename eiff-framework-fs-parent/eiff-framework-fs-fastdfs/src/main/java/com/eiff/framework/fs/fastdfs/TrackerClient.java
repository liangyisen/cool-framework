...
class TrackerClient {
	 ...
	 */
	TrackerClient(NonGlobalConfig config) {
		this.config = config;
	}
	 ...
	 */
	TrackerServer getConnection() throws IOException {
		return this.config.getTrackerGroup().getConnection();
	}
	 ...
	 */
	StorageServer getStoreStorage(TrackerServer trackerServer) throws IOException {
	 ...
	 */
	StorageServer getStoreStorage(TrackerServer trackerServer, String groupName) throws IOException {
		byte[] header;
		String ip_addr;
		int port;
		byte cmd;
		...
		try {
			...
			if (groupName != null && groupName.length() > 0) {
				...
				bs = groupName.getBytes(this.config.getCharset());
			...
			return new StorageServer(ip_addr, port, store_path, this.config);
	...
	StorageServer[] getStoreStorages() throws IOException {
		return this.getStoreStorages(null, null);
	}

	ServerInfo[] getStorages() throws IOException {
		byte[] header;
		String ip_addr;
		int port;
		byte cmd;
		...
		try {
			...
			for (int i = 0; i < serverCount; i++) {
				...
				results[i] = new ServerInfo(ip_addr, port, store_path);
			}
			return results;
		} catch (IOException ex) {
			throw ex;
	 ...
	 */
	StorageServer[] getStoreStorages(TrackerServer trackerServer, String groupName) throws IOException {
		byte[] header;
		String ip_addr;
		int port;
		byte cmd;
		...
		try {
			...
			for (int i = 0; i < serverCount; i++) {
				...
				results[i] = new StorageServer(ip_addr, port, store_path, this.config);
			}
	 ...
	 * @return storage server Socket object, return null if fail
	 */
	String getFetchStorageInfo(TrackerServer trackerServer, String fileId) throws IOException {
		String[] parts = new String[2];
		this.errno = StorageCommand.splitFileId(fileId, parts);
		if (this.errno != 0) {
			return null;
		}
		...
		if (servers == null) {
			...
			return servers[0].getIpAddr() + ":" + servers[0].getPort();
		}
	}
	 ...
	 * @return storage server Socket object, return null if fail
	 */
	StorageServer getFetchStorage(TrackerServer trackerServer, String groupName, String filename) throws IOException {
		ServerInfo[] servers = this.getStorages(trackerServer, ProtoCommon.TRACKER_PROTO_CMD_SERVICE_QUERY_FETCH_ONE,
				groupName, filename);
		if (servers == null) {
			return null;
		} else {
			return new StorageServer(servers[0].getIpAddr(), servers[0].getPort(), 0, this.config);
		}
	}

	String getUpdateStorageInfo(TrackerServer trackerServer, String fileId) throws IOException {
		String[] parts = new String[2];
		this.errno = StorageCommand.splitFileId(fileId, parts);
		if (this.errno != 0) {
			return null;
	 ...
	 * @return storage server Socket object, return null if fail
	 */
	StorageServer getUpdateStorage(TrackerServer trackerServer, String groupName, String filename) throws IOException {
		ServerInfo[] servers = this.getStorages(trackerServer, ProtoCommon.TRACKER_PROTO_CMD_SERVICE_QUERY_UPDATE,
				groupName, filename);
		if (servers == null) {
			return null;
	...
	StorageServer getUpdateStorage(TrackerServer trackerServer, String file_id) throws IOException {
		String[] parts = new String[2];
		...
		return this.getUpdateStorage(trackerServer, parts[0], parts[1]);
	}
	 ...
	 * @return storage servers, return null if fail
	 */
	ServerInfo[] getFetchStorages(TrackerServer trackerServer, String groupName, String filename) throws IOException {
	...
	protected ServerInfo[] getStorages(TrackerServer trackerServer, byte cmd, String groupName, String filename)
		...
		try {
			bs = groupName.getBytes(this.config.getCharset());
			bGroupName = new byte[ProtoCommon.FDFS_GROUP_NAME_MAX_LEN];
			bFileName = filename.getBytes(this.config.getCharset());
	 ...
	 * @return storage server Socket object, return null if fail
	 */
	StorageServer getFetchStorage1(TrackerServer trackerServer, String file_id) throws IOException {
		String[] parts = new String[2];
		this.errno = StorageCommand.splitFileId(file_id, parts);
		if (this.errno != 0) {
			return null;
	 ...
	 * @return storage servers, return null if fail
	 */
	ServerInfo[] getFetchStorages1(TrackerServer trackerServer, String file_id) throws IOException {
		String[] parts = new String[2];
		this.errno = StorageCommand.splitFileId(file_id, parts);
		if (this.errno != 0) {
			return null;
	 ...
	 */
	StructGroupStat[] listGroups() throws IOException {
		byte[] header;
		boolean bNewConnection;
		Socket trackerSocket;
	...
	StructStorageStat[] listStorages() throws IOException {
		List<StructStorageStat> storages = new ArrayList<>();
		StructGroupStat[] listGroups = this.listGroups();
		for (int i = 0; i < listGroups.length; i++) {
			StructStorageStat[] listStorages = this.listStorages(null, listGroups[i].getGroupName(), null);
			for (StructStorageStat storage : listStorages) {
				storages.add(storage);
			}
		}
		StructStorageStat[] array = Arrays.copyOf(storages.toArray(new StructStorageStat[] {}), storages.size());
	 ...
	 */
	StructStorageStat[] listStorages(TrackerServer trackerServer, String groupName) throws IOException {
	...
	StructStorageStat[] listStorages(TrackerServer trackerServer, String groupName, String storageIpAddr)
		...
		try {
			...
			if (storageIpAddr != null && storageIpAddr.length() > 0) {
				bIpAddr = storageIpAddr.getBytes(this.config.getCharset());
	...
	private boolean deleteStorage(TrackerServer trackerServer, String groupName, String storageIpAddr)
		...
		int ipAddrLen;
		byte[] bIpAddr = storageIpAddr.getBytes(this.config.getCharset());
		if (bIpAddr.length < ProtoCommon.FDFS_IPADDR_SIZE) {
			ipAddrLen = bIpAddr.length;
		} else {
			ipAddrLen = ProtoCommon.FDFS_IPADDR_SIZE - 1;
	 ...
	 * @return true for success, false for fail
	 */
	boolean deleteStorage(String groupName, String storageIpAddr) throws IOException {
		return this.deleteStorage(this.config.getTrackerGroup(), groupName, storageIpAddr);
	}
	 ...
	 * @return true for success, false for fail
	 */
	boolean deleteStorage(TrackerGroup trackerGroup, String groupName, String storageIpAddr) throws IOException {
		int serverIndex;
		int notFoundCount;
